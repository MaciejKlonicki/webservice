...
const NavigationBar = ({ t }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setState({ scrolled: scrollPosition > 0 })
    }
        ...
        <Nav style={{ position: "absolute", right: 5 }}>
          ...
          >
            ...
            <NavDropdown.Item href='login'>
              {localStorage.getItem("email") !== null ? <div><Nav.Link href="/login" onClick={logout}><BiLogOut style={{ position: "relative", bottom: "1px" }} />{' '}{t('Logout.1')}</Nav.Link></div> : null}
            </NavDropdown.Item>
          </NavDropdown>
        </Nav>
      </Navbar.Collapse>
  ...
  return (
    <div>
      <Navbar
        ...
        style={{
          paddingRight: '550px',
          position: state.scrolled ? 'fixed' : 'relative',
          width: '100%',
          top: 0,
          zIndex: 1000,
        }}
